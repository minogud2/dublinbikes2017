...
class Connection():
	...
	def read_from_dublinbike(self):
		...
		try:
			...
			print("Dublin bike connection succeeded")
		except IOError:
	...
	def read_from_weather(self):
		...
		try:
			...
			print("Weather connection succeeded")
		#self.__date=unix
		except IOError:
	...
	def make_table(self):
		...
		with self.__con:
			...
			cur.execute("CREATE TABLE DynamicTest(num INT, last_update VARCHAR(55), status TEXT, available_bikes INT, bike_stands INT, available_bike_stands INT, banking BOOLEAN,FOREIGN KEY(num) REFERENCES static(num))")
	...
	def add_data_to_dynamic_table(self):
		...
		for i in range(0, len(self.__data)):
			...
			(number[i],time.strftime("%a, %d %b %Y %H:%M:%S ", time.gmtime(last_update[i]/1000)), status[i], available_bikes[i], bike_stands[i],available_bike_stands[i],banking[i]))
			self.__con.commit()
		cur.close()
		self.__con.close()
